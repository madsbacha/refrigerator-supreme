        ...
        <div v-for="item in items" v-bind:key="item.id" class="w-1/4">
          <div class="shadow rounded m-3 bg-white">
            ...
            <div class="flex w-full justify-between">
              ...
              <input @input="handleRating" v-bind:item-id="item.id" v-once :value="ratingOn(item.id)" type="number" placeholder="#" min="1" max="10" class="border-l border-grey-100 py-2 px-3 rounded-br w-16" />
            </div>
          </div>
        </div>
      </div>
