    ...
    <div class="bg-gray-200 flex justify-center h-screen content-center flex-shrink">
        <div v-on:keydown.enter.capture="handleSubmit" class="w-9/12 sm:w-2/3 md:w-1/2 lg:w-1/3 xl:w-1/4 pt-64">
            ...
            <input v-model="email" type="email" placeholder="Email" required
                   class="block shadow py-2 px-3 border-gray-200 border-solid border-b rounded rounded-b-none w-full"/>
            <input v-model="password" type="password" placeholder="Password" required
                   class="block shadow mb-2 py-2 px-3 rounded rounded-t-none w-full"/>
            <p class="text-sm text-red-600">{{ errorMessage }}</p>
            <button v-on:click="handleSubmit" class="bg-green-600 text-white mt-2 py-2 px-4 rounded hover:bg-green-500"
                    v-text="submitText"></button>
            <p class="inline ml-3">or
                <button v-on:click="toggleView" class="text-blue-600 hover:underline hover:text-blue-500"
                        v-text="alternativeText"></button>
            </p>
        </div>
    </div>
</template>
